...
ROUND(SUM(OD.quantity * P.price)*100 / SUM(SUM(OD.quantity * P.price)) OVER (),2) AS Pecentage_contribution
FROM order_details OD
JOIN pizzas P
ON OD.pizza_id = P.pizza_id
JOIN pizza_types PT
ON PT.pizza_type_id = P.pizza_type_id
GROUP BY PT.category
ORDER BY Pecentage_contribution DESC;
...
SELECT O.date,
ROUND(SUM(OD.quantity * P.price),0) AS Revenue,
ROUND(SUM(SUM(OD.quantity * P.price)) OVER(ORDER BY O.date),0) AS Cummulative_Revenue
FROM orders O
JOIN
order_details OD
ON O.order_id = OD.order_id
...
SELECT PT.category, ROUND(SUM(OD.quantity * P.price),0) AS Revenue
FROM order_details OD
JOIN 
pizzas P
ON P.pizza_id = OD.pizza_id
